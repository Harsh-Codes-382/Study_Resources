// src/components/LldTracker.jsx
import { useMemo, useState } from "react";
import { Link } from "react-router-dom";
import { ArrowLeft, ExternalLink } from "lucide-react";
import Layout from "./Layout";
import { LLD_TIERS } from "../data/lldTracker";
import "./HldTracker.css"; // shared .trk-* styles
import "./LldTracker.css"; // green-accent overrides
...
export default function LldTracker() {
  // Initial statuses come from the data file. Editing lldTracker.js is what
  ...
  const all = LLD_TIERS.reduce((a, t) => a + t.topics.length, 0);
  let done = 0,
    doing = 0;
  ...
  return (
    <Layout headRight={<p className="nb-tag">{done}/{all} problems</p>}>
      <div className="trk lld-scope">
        <Link className="nb-back" to="/lld">
          <ArrowLeft size={15} /> LLD notes
        </Link>

        <div className="trk-head">
          <p className="trk-eyebrow">Object Design · LLD Roadmap</p>
          <h1 className="trk-title">LLD Study Tracker</h1>
          <p className="trk-sub">
            {all} problems across three tiers. Tap any node to mark it as to do,
            doing, or done.
          </p>
              ...
              {topics.map((t, i) => {
                ...
                return (
                  <div className={`trk-row ${st}`} key={k}>
                    ...
                    {t.note && (
                      <Link className="trk-note-link" to={`/lld/${t.note}`}>
